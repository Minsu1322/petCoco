...
const ChangeProfileModal = ({ clickModal }: ModalProps) => {
  ...
  const submitChange = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    await updateProfileNickNameWithSupabase(nickName, user.data?.id);
    //유저 닉네임 변경
    if (profileImage === null) {
      ...
      await updateProfileImgWithSupabase(publicUrl.data.publicUrl, user.data?.id);
    } //유저 프로필 사진 변경

    queryClient.invalidateQueries({ queryKey: ["user"] });

    // let updatingData = user.data;
    ...
    // if (!profileImage) {
    //   return;
    // } else if (profileImage !== user.data.profile_img) {
    //  updatingData.profile_img = profileImage
    //  }
