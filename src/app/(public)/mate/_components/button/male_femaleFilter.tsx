...
const Male_femaleFilter = ({onSelect, selectedGender, setSelectedGender }: Male_femaleFilterProps) => {
  ...
  return (
    ...
    <div className="flex justify-between">
      <div
        className={`px-[4.06rem] py-[1rem] rounded-[1rem] text-[1rem] cursor-pointer  font-[500]  ${selectedGender === "수" ? 'bg-mainColor text-white ' : 'bg-[#EFEFF0] text-[#999]' }`}
        onClick={() => handleSelect('수')}
      >
        남아
      </div>
      <div
        className={`px-[4.06rem] py-[1rem] rounded-[1rem] text-[1rem] cursor-pointer text-[#999] font-[500]  ${selectedGender === "암" ? 'bg-mainColor text-white' : 'bg-[#EFEFF0] text-[#999]' }`}
